...
from env.constants import ( GRID_WIDTH, GRID_HEIGHT, DANGEROUS_FIRE_STATES, FIREPROOF_TERRAIN, FIRE_LOW, IGNITION_PROB, BURN_DURATION, FIRE_ASH, FIRE_HIGH, FIRE_MID, FIRE_NONE )
...
def _advance_burning_cells(terrain, fire_state,  new_fire_state, new_burn_timer):
    ys, xs = np.where(np.isin(fire_state, list(DANGEROUS_FIRE_STATES)))
